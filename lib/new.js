  ...
  inquirer.prompt(questions).then(function (answers) {
    ...
    setTargetPath(function (target) {
      ...
      try {
        ...
        console.log(chalk.green(`added success：${path.join(target, fileName + '.js')}`));
      } catch (e) {
      }
      ...
      try {
        ...
        console.log(chalk.green(`added success：${path.join(target, fileName + '.less')}`));
      } catch (e) {
      }
    }, 'page');
  ...
  inquirer.prompt(questions).then(function (a1) {
    ...
    inquirer.prompt(q).then(function (a2) {
      setTargetPath(function (target) {
        ...
        console.log(chalk.green(`added component ${a2.name} success：${cPath}`));
      }, 'component');
    });
  });
}
  ...
  inquirer.prompt(questions).then(function (answers) {
    ...
    setTargetPath(function (target) {
      try {
        ...
        console.log(chalk.green(`added model ${name} success：${path.join(target, name + '.js')}`));
      } catch (e) {
        console.log(chalk.red(`added model ${name} fail`, e));
      }
    }, 'model');
  });
}
  ...
  inquirer.prompt(questions).then(function (answers) {
    ...
    setTargetPath(function (target) {
      try {
        ...
        console.log(chalk.green(`added service ${name} success：${path.join(target, name + '.js')}`));
      } catch (e) {
        console.log(chalk.red(`added service ${name} fail`, e));
      }
    }, 'service');
  });
}
